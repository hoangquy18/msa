...
from datasets.datasets_synapse import Synapse_dataset, RandomGenerator, ISIC_dataset
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(train_bar):
            ...
            if iter_num % 100 == 0:
                ...
                logging.info(
                    "iteration %d : loss_boundary: %f" % (iter_num, acc_loss_bo)
                )
                # acc_loss = 0.0
                # acc_loss_ce = 0.0
                # acc_loss_dc = 0.0
        ...
        if epoch_num >= int(max_epoch / 2) and (epoch_num + 1) % eval_interval == 0:
            ...
            mean_dice, mean_hd95 = inference(
                model, testloader, args, test_save_path=test_save_path
            )
